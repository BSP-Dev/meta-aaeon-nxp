...
echo "IMAGE_INSTALL:append = \"pm-utils lshw memtester gptfdisk rsync wireless-tools \"" >> $BUILD_DIR/conf/local.conf
echo "IMAGE_INSTALL:append = \"ntfs-3g libmnl tpm2-tools libmodbus networkmanager sysbench \"" >> $BUILD_DIR/conf/local.conf
echo "IMAGE_INSTALL:append = \"stress-ng hdparm cmake ca-certificates devmem2 spitools gcc \"" >> $BUILD_DIR/conf/local.conf
echo "IMAGE_INSTALL:append = \"python3-python-vlc git doxygen libp11 dbus tpm2-openssl \"" >> $BUILD_DIR/conf/local.conf
echo "IMAGE_INSTALL:append = \"openssl-tpm-engine autoconf-archive json-c json-glib \"" >> $BUILD_DIR/conf/local.conf
...
echo "IMAGE_INSTALL:append = \"libgpiod libgpiod-dev libgpiod-tools dhcpcd wpa-supplicant \"" >> $BUILD_DIR/conf/local.conf
...
echo "IMAGE_INSTALL:append = \"v4l-utils mtools lmsensors modemmanager minicom python3-speedtest-cli read-edid edid-decode aaeon-tools service-tools\"" >> $BUILD_DIR/conf/local.conf
